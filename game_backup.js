...
// Camera (Roblox-style third-person)
let cameraDistance = 8; // Distance behind player
let cameraHeight = 3; // Height above player
let cameraAngleX = 0; // Horizontal rotation
let cameraAngleY = 0.3; // Vertical rotation (pitch)
let minCameraDistance = 3;
let maxCameraDistance = 15;
...
function init() {
    ...
    scene.fog = new THREE.Fog(0x0a0a0a, 5, 50); // Less fog, can see further!

    // Camera (Third-person Roblox style)
    ...
    // Lighting - MUCH BRIGHTER so you can see!
    const ambientLight = new THREE.AmbientLight(0x404040, 1.5); // Much brighter ambient
    scene.add(ambientLight);

    // Add ceiling lights throughout the facility
    const ceilingLightPositions = [
        { x: 0, z: 0 },     // Office
        { x: 15, z: 0 },    // East hallway
        { x: -15, z: 0 },   // West area
        { x: 0, z: 15 },    // North hallway
        { x: 0, z: -15 },   // South area
        { x: 10, z: 10 },   // Northeast
        { x: -10, z: -10 }, // Southwest
        { x: 10, z: -10 },  // Southeast
    ...
    ceilingLightPositions.forEach(pos => {
        const light = new THREE.PointLight(0xffffaa, 1.2, 20);
    ...
    // Orange hallway lights (FNAF style)
    const hallwayLights = [
        { x: -3.5, z: 0, side: 'left' },  // Left doorway
        { x: 3.5, z: 0, side: 'right' }    // Right doorway
    ];

    hallwayLights.forEach(pos => {
...
function createEnvironment() {
    ...
    // FNAF Office Layout
    // Central Office (where player starts)
    createFNAFOffice();
...
function onMouseMove(event) {
    ...
    // Roblox-style: Rotate camera around player
    cameraAngleX -= movementX * 0.003; // Horizontal rotation
    cameraAngleY -= movementY * 0.003; // Vertical rotation
...
function startGame() {
    ...
    // Reset player (start in office, facing forward)
...
function updatePlayer(delta) {
    ...
    if (direction.length() > 0) {
        ...
        // Roblox-style: Movement is relative to camera view
        const cameraRotation = new THREE.Quaternion();
        cameraRotation.setFromAxisAngle(new THREE.Vector3(0, 1, 0), cameraAngleX);
        ...
        if (!checkCollision(newPosition)) {
            ...
            // Make player character face movement direction
    ...
    if (isMobile && touchControls.look.active) {
        cameraAngleX -= touchControls.look.x * 0.05;
        cameraAngleY -= touchControls.look.y * 0.05;
        cameraAngleY = Math.max(-Math.PI / 3, Math.min(Math.PI / 3, cameraAngleY));
    }
    ...
    // Update camera position (Roblox-style: orbit around player)
    updateCamera();
...
function updateCamera() {
    ...
    const offsetX = Math.sin(cameraAngleX) * cameraDistance * Math.cos(cameraAngleY);
    const offsetZ = Math.cos(cameraAngleX) * cameraDistance * Math.cos(cameraAngleY);
    const offsetY = Math.sin(cameraAngleY) * cameraDistance + cameraHeight;
    ...
    camera.lookAt(player.position.x, player.position.y + cameraHeight * 0.8, player.position.z);

    // Update flashlight position and direction
    if (flashlight.on) {
        flashlight.light.position.copy(camera.position);
        const flashlightDirection = new THREE.Vector3();
        flashlightDirection.subVectors(player.position, camera.position).normalize();
        flashlight.light.target.position.copy(camera.position).add(flashlightDirection.multiplyScalar(5));
    }
}
